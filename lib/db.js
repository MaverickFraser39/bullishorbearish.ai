...
export function listTakes(startupId, { limit = 100, offset = 0 } = {}) {
  return db.prepare(`
    SELECT id, direction, reason, author, updated_at
    FROM votes
...
export function fileReport({ startupId, voteId, reason, reporter }) {
  db.prepare(
    'INSERT INTO reports (startup_id, vote_id, reason, reporter) VALUES (?, ?, ?, ?)'
  ).run(startupId, voteId ?? null, reason ?? null, reporter);
}

/** Reports grouped by company, busiest first — the moderation queue. */
export function listReports({ limit = 100 } = {}) {
  return db.prepare(`
    SELECT r.id, r.reason, r.created_at, r.vote_id,
           s.slug, s.name,
           v.direction AS take_direction, v.reason AS take_reason, v.author AS take_author
    FROM reports r
    JOIN startups s ON s.id = r.startup_id
    LEFT JOIN votes v ON v.id = r.vote_id
    ORDER BY r.created_at DESC
    LIMIT ?
  `).all(limit);
}

/** Removes a company and everything attached to it. */
export function deleteStartup(slug) {
  const row = db.prepare('SELECT id FROM startups WHERE slug = ?').get(slug);
  if (!row) return false;
  db.prepare('DELETE FROM startups WHERE id = ?').run(row.id);
  return true;
}

/** Strips the text from one take, leaving the vote counted. */
export function redactTake(voteId) {
  const res = db.prepare(
    "UPDATE votes SET reason = NULL, author = NULL WHERE id = ?"
  ).run(voteId);
  return res.changes > 0;
}

export function getStartupById(id) {
  const row = db.prepare('SELECT slug FROM startups WHERE id = ?').get(id);
  return row ? getStartup(row.slug) : null;
}
